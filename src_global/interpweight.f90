...
! Getting longitudes/latitudes from input file
    ...
    IF (is_root_prc) THEN
      ...
      IF (ALLOC_ERR /= 0) CALL ipslerr(3,'interpweight_2D','Problem in allocation of variable resol_in','','')
    END IF
...
! Getting longitudes/latitudes from input file
    ...
    IF (is_root_prc) THEN
      ALLOCATE(resol_in(iml,jml,2), STAT=ALLOC_ERR)
      IF (ALLOC_ERR /= 0) CALL ipslerr(3,'interpweight_4D','Problem in allocation of variable resol_in','','')
    END IF
...
! Getting longitudes/latitudes from input file
    ...
    IF (is_root_prc) THEN
      ALLOCATE(resol_in(iml,jml,2), STAT=ALLOC_ERR)
      IF (ALLOC_ERR /= 0) CALL ipslerr(3,'interpweight_4Dcont','Problem in allocation of variable resol_in','','')
    END IF
      ...
      CASE ('default')
        ...
        DO ib=1,npt
          ...
          IF (SUM(ovar(ib,:)) > 1.0000001) THEN
            ...
            CALL ipslerr(3,'interpweight_provide_fractions1D',TRIM(msg),'','')
          END IF
        END DO
      ...
      CASE ('default')
        ...
        DO ib=1,npt
          ...
          IF (SUM(ovar(ib,:)) > 1.0000001) THEN
            ...
            END DO
            msg='total of fractions above 1.!'
            CALL ipslerr(3,'interpweight_provide_fractions2D',TRIM(msg),'','')
          END IF
        END DO
...
!
          IF (SUM(ovar(ib,:)) > 1.0000001) THEN
            ...
            END IF
            msg='total of fractions above 1.!'
            CALL ipslerr(3,'interpweight_provide_fractions3D',TRIM(msg),'','')
         END IF
...
! Following: http://stackoverflow.com/questions/3828094/function-returning-an-array-in-fortran
    ...
    IF (is_root_prc) THEN
       ...
       IF (rcode /= NF90_NOERR) CALL ipslerr(3, 'interpweight_get_var2dims_file', 'Error in ng90_open', &
            TRIM(nf90_strerror(rcode)),'')

       rcode = nf90_inq_varid(nid, varname, vid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3, 'interpweight_get_var2dims_file', 'Error in ng90_inq_varid', &
            TRIM(nf90_strerror(rcode)),'')

       rcode = nf90_inquire_variable(nid, vid, NDIMS = Ndims)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3, 'interpweight_get_var2dims_file', 'Error in ng90_inquire_variable 1', &
            TRIM(nf90_strerror(rcode)),'')
       
       IF (Ndims /= 2) THEN
          msg = "variable '" // TRIM(varname) // "' has not 2 dimensions!!"
          CALL ipslerr(3,'interpweight_get_var2dims_file',TRIM(msg), '', '')
       END IF
       
       rcode = nf90_inquire_variable(nid, vid, DIMIDS = dimsid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3, 'interpweight_get_var2dims_file', 'Error in ng90_inquire_variable 2', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(1), LEN = interpweight_get_var2dims_file(1))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3, 'interpweight_get_var2dims_file', 'Error in ng90_inquire_dimension 1', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(2), LEN = interpweight_get_var2dims_file(2))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3, 'interpweight_get_var2dims_file', 'Error in ng90_inquire_dimension 2', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = NF90_CLOSE(nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3, 'interpweight_get_var2dims_file', 'Error in ng90_close', &
            TRIM(nf90_strerror(rcode)),'')
    END IF
...
!_================================================================================================================================
  FUNCTION interpweight_get_var3dims_file(filename, varname)
    ...
    IF (is_root_prc) THEN
       ! Open file for read only mode
       rcode = nf90_open(TRIM(filename), NF90_NOWRITE, nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_open', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inq_varid(nid, varname, vid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_inq_varid', &
            TRIM(nf90_strerror(rcode)),'')

       rcode = nf90_inquire_variable(nid, vid, NDIMS = Ndims)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_inquire_variable 1', &
            TRIM(nf90_strerror(rcode)),'')
       
       IF (Ndims /= 3) THEN
          msg = "variable '" // TRIM(varname) // "' has not 3 dimensions!!"
          CALL ipslerr(3,'interpweight_get_var3dims_file',TRIM(msg), '', '')
       END IF
       
       rcode = nf90_inquire_variable(nid, vid, DIMIDS = dimsid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_inquire_variable 2', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(1), LEN = interpweight_get_var3dims_file(1))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_dimesion 1', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(2), LEN = interpweight_get_var3dims_file(2))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_dimension 2', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(3), LEN = interpweight_get_var3dims_file(3))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_dimension 3', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = NF90_CLOSE(nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var3dims_file', 'Error in ng90_close', &
            TRIM(nf90_strerror(rcode)),'')
    END IF
...
!_================================================================================================================================
  FUNCTION interpweight_get_var4dims_file(filename, varname)
    ...
    IF (is_root_prc) THEN
       ! Open file for read only mode
       rcode = nf90_open(TRIM(filename), NF90_NOWRITE, nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_open', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inq_varid(nid, varname, vid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_inq_varid', &
            TRIM(nf90_strerror(rcode)),'')

       rcode = nf90_inquire_variable(nid, vid, NDIMS = Ndims)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_inquire_variable 1', &
            TRIM(nf90_strerror(rcode)),'')

       IF (Ndims /= 4) THEN
          msg = "variable '" // TRIM(varname) // "' has not 4 dimensions!!"
          CALL ipslerr(3,'interpweight_get_var4dims_file',TRIM(msg), '', '')
       END IF

       rcode = nf90_inquire_variable(nid, vid, DIMIDS = dimsid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_inquire_variable 2', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(1), LEN = interpweight_get_var4dims_file(1))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_inquire_dimension 1', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(2), LEN = interpweight_get_var4dims_file(2))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_inquire_dimension 2', &
            TRIM(nf90_strerror(rcode)),'')

       rcode = nf90_inquire_dimension(nid, dimsid(3), LEN = interpweight_get_var4dims_file(3))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_inquire_dimension 3', &
            TRIM(nf90_strerror(rcode)),'')
       
       rcode = nf90_inquire_dimension(nid, dimsid(4), LEN = interpweight_get_var4dims_file(4))
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_inquire_dimension 4', &
            TRIM(nf90_strerror(rcode)),'')

       rcode = NF90_CLOSE(nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_var4dims_file','Problem with nf90_close', &
            TRIM(nf90_strerror(rcode)),'')
    END iF
...
!_================================================================================================================================
  INTEGER FUNCTION interpweight_get_varNdims_file(filename, varname)
    ...
    IF (is_root_prc) THEN
       ...
       ! Open file for read only mode
       rcode = nf90_open(TRIM(filename), NF90_NOWRITE, nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_varNdims_file','Problem with nf90_open', &
            TRIM(nf90_strerror(rcode)), '')
       
       rcode = nf90_inq_varid(nid, varname, vid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_varNdims_file','Problem with nf90_inq_varid', &
            TRIM(nf90_strerror(rcode)), '')
       
       rcode = nf90_inquire_variable(nid, vid, NDIMS = interpweight_get_varNdims_file)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_varNdims_file','Problem with nf90_inquire_variable', &
            TRIM(nf90_strerror(rcode)), '')

       rcode = NF90_CLOSE(nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'interpweight_get_varNdims_file','Problem with nf90_close', &
            TRIM(nf90_strerror(rcode)), '')
    END IF
...
LOGICAL FUNCTION isin_file(filename, varname)
    ...
    IF (is_root_prc) THEN
       rcode = nf90_open(TRIM(filename), NF90_NOWRITE, nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'isin_file','Problem with nf90_open',TRIM(nf90_strerror(rcode)),'')

       rcode = nf90_inquire(nid, Ndims, Nvars)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'isin_file','Problem with nf90_inquire',TRIM(nf90_strerror(rcode)),'')

       DO iv=1, Nvars
          rcode = nf90_inquire_variable(nid, iv, name=varinfile)
          IF (rcode /= NF90_NOERR) CALL ipslerr(3,'isin_file','Problem with nf90_inquire_variable',TRIM(nf90_strerror(rcode)),'')
       ...
       rcode = NF90_CLOSE(nid)
       IF (rcode /= NF90_NOERR) CALL ipslerr(3,'isin_file','Problem with nf90_close',TRIM(nf90_strerror(rcode)),'')
    END IF
