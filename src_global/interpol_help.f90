...
!
!
!< $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE_2_2/ORCHIDEE/src_global/interpol_help.f90 $
...
!
  SUBROUTINE aggregate_vec_p(nbpt, lalo, neighbours, resolution, contfrac,          &
    ...
    INTEGER(i_std), INTENT(in)   :: neighbours(nbpt,NbNeighb)   
    ...
    ELSE IF ( grid_type == regular_xy ) THEN
       IF ( proj_stack(1)%code > undef_int-1 ) THEN
          CALL ipslerr(3, "aggregate_vec_p", "regular_xy projection was not intialized.", &
    ...
    ELSE
       CALL ipslerr(3, "aggregate_vec_p", "Interpolation is only possible for regular lat/lon grids for the moment.", &
            &       "Use interpolations with XIOS for unstructured grid","Set XIOS_INTERPOLATION=y in run.def")
    ENDIF
    !
    CALL BCAST(ok)
