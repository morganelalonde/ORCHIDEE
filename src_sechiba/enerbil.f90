...
!! enerbil_clear, enerbil_begin, enerbil_surftemp, enerbil_flux and enerbil_evapveg
!!
!!\n DESCRIPTION                                : 
!! \n
!! \latexonly 
...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE_2_2/ORCHIDEE/src_sechiba/enerbil.f90 $
!! $Date: 2021-07-30 18:36:29 +0200 (Fri, 30 Jul 2021) $
!! $Revision: 7265 $
!! \n
!_ ================================================================================================================================
...
MODULE enerbil
  ...
  PRIVATE
  PUBLIC :: enerbil_main, enerbil_initialize, enerbil_finalize, enerbil_clear
...
!_ ================================================================================================================================
 ...
 &           'pdqsold too high ','','')
  ...
  !!
  !! Since the explicit snow scheme is adapted, the notion of skin layer in ORCHIDEE is abandoned and
  ...
  SUBROUTINE enerbil_surftemp (kjpindex, zlev, emis, epot_air, &
    ...
    REAL(r_std)                      :: speed
    REAL(r_std),DIMENSION (kjpindex) :: dtes        !! Diagnostic output variable for the change of the thermal energy content of surface for which the energy balance is calculated (J m-2)
...
!_ ================================================================================================================================
    ...
    !! 4. Solution of the energy balance
      ...
      ! Calculate dtes only for diagnostic output and send it to xios
      dtes(ji) = (soilcap(ji)/cp_air)*dtheta/dt_sechiba

    ENDDO

    ! Send diagnostic variable to XIOS
    CALL xios_orchidee_send_field("dtes", dtes)
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE enerbil_flux (kjpindex, emis, temp_sol, rau, u, v, q_cdrag, vbeta, vbeta1, vbeta5, &
    ...
    REAL(r_std), DIMENSION (kjpindex)                            :: betatot           !! beta used in the computation of qsurf  (-) 
!_ ================================================================================================================================
    ...
    !! 2. Calculation of the net upward flux of longwave radiation
      ...
      fluxlat(ji) = chalsu0 * rau(ji) * qc * vbeta1(ji) * (un - vbeta5(ji)) * &
       ...
       WRITE (numout,*) 'DEBUG MORGANE 2 fluxlat: fluxlat=', fluxlat(ji), ' chalsu0=', chalsu0, ' chalev0=', chalev0
       WRITE (numout,*) '  rau=', rau(ji), ' qc=', qc, ' vbeta1=', vbeta1(ji), ' vbeta5=', vbeta5(ji), ' vbeta=', vbeta(ji)
       WRITE (numout,*) '  un=', un, ' qsol_sat_new=', qsol_sat_new(ji), ' qair=', qair(ji)
      ...
      WRITE (numout,*) 'MORGANE TEST DEBUG 1: fluxsens=', fluxsens(ji), ' rau=', rau(ji), ' psnew=', psnew(ji), ' epot_air=', epot_air(ji)
