...
!!
!! SVN              :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE_2_2/ORCHIDEE/src_sechiba/condveg.f90 $
...
CONTAINS
  ...
  SUBROUTINE condveg_xios_initialize
    ...
    IF (xios_interpolation .AND. alb_bg_modis .AND. restname_in=='NONE') THEN
...
!_ ================================================================================================================================
    ...
    CALL condveg_frac_snow(kjpindex, snow_nobio, snowrho, snowdz, &
                           frac_snow_veg, frac_snow_nobio)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE condveg_albedo  (kjpindex,       veget,         veget_max,     drysoil_frac, frac_nobio,       &
    ...
    REAL(r_std), DIMENSION(kjpindex)                    :: tot_bare_soil_notree  !! Total bare soil fraction without accounting for Trees
...
!_ ================================================================================================================================
    ...
    ELSE ! calculated following Chalita and Treut (1994)
       ...
       !
       ! Define a new variable (tot_bare_soil_notree) to quantify the total bare soil fraction outside the forest PFTs
       !
       tot_bare_soil_notree(:) = zero
       DO ji  = 1, kjpindex
          DO jv = 1, nvm
             IF ( (fraction_veg(ji) .GT. min_sechiba) .AND. (.NOT. is_tree(jv) ) )  THEN
                tot_bare_soil_notree(ji) = tot_bare_soil_notree(ji) + veget_max(ji,jv) - veget(ji,jv)
             ENDIF
          ENDDO
       ENDDO
       !
       ! Treat the albedo for the equivalent bare soil fraction that do not account for trees
       DO jb = 1, 2
          DO ji = 1, kjpindex
             IF ( fraction_veg(ji) .GT. min_sechiba ) THEN
                snowa_veg(ji,jb) = snowa_veg(ji,jb) + &
                     tot_bare_soil_notree(ji)/fraction_veg(ji) * ( snowa_aged_tmp(1,jb)+snowa_dec_tmp(1,jb)*agefunc_veg(ji) )
             ENDIF
          ENDDO
       ENDDO                
       ! 
       ! For all PFTs (except bare soil) we use veget_max for the Trees and else veget
       DO jb = 1, 2
          DO jv = 2, nvm
             IF (is_tree(jv)) THEN
                DO ji = 1, kjpindex
...
!_ ================================================================================================================================
  ...
  SUBROUTINE condveg_frac_snow(kjpindex, snow_nobio, snowrho, snowdz, &
                               frac_snow_veg, frac_snow_nobio)
    !! 0. Variable and parameter declaration
    !! 0.1 Input variables
    INTEGER(i_std), INTENT(in)                          :: kjpindex        !! Domain size
...
!_ ================================================================================================================================
  ...
  IF (grid_type==unstructured) THEN
     CALL ipslerr_p(3,'condveg_soilalb','Reading of SOILALB_FILE must be implemented with XIOS to be used for unstructured grid.', &
             'Use option alb_bg_modis for unstructured grid for now.','')
  END IF
...
!_ ================================================================================================================================
    ...
    IF (xios_interpolation) THEN
       ...
       CALL xios_orchidee_recv_field('bg_alb_vis_interp',soilalb_bg(:,ivis))
       CALL xios_orchidee_recv_field('bg_alb_nir_interp',soilalb_bg(:,inir))

       aalb_bg(:)=1
       
    ELSE
       ! Read background albedo file using IOIPSL and interpolate using aggregate standard method
       ...
       IF (printlev_loc >= 2) WRITE(numout,*) "condveg_background_soilalb: Start interpolate " &
            // TRIM(filename) // " for variable " // TRIM(variablename)
    ...
    CALL xios_orchidee_send_field("interp_diag_alb_vis",soilalb_bg(:,ivis))
    CALL xios_orchidee_send_field("interp_diag_alb_nir",soilalb_bg(:,inir))
...
!_ ================================================================================================================================
  ...
  SUBROUTINE condveg_alb_urban(nbpt, lalo, neighbours,  resolution, contfrac)
