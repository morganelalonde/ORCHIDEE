...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE_2_2/ORCHIDEE/src_sechiba/routing.f90 $
...
MODULE routing
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: routing_main, routing_initialize, routing_finalize, routing_clear, routing_names, routing_xios_initialize
...
!_ ================================================================================================================================
  ...
  !!  =============================================================================================================================
  !! SUBROUTINE:    routing_xios_initialize
  !!
  ...
  END SUBROUTINE routing_xios_initialize
...
!! LOCAL VARIABLES
    ...
    REAL(r_std), DIMENSION(nbpt, nbasmax)        :: netflow_stream            !! Input - Output flow to stream reservoir
    REAL(r_std), DIMENSION(nbpt, nbasmax)        :: netflow_fast              !! Input - Output flow to fast reservoir
    ...
    DO icc = 1, num_largest
       ...
       IF ( name_found > 1 ) THEN
          DO ic=num_largest,1,-1
             IF ( TRIM(sorted_names(icc)) == TRIM(sorted_names(ic)) .AND. name_found > 1 ) THEN
                IF ( name_found < 10 ) THEN
                   WRITE(nn,'(I1)')  name_found
                ELSE IF ( name_found < 100 ) THEN
                   WRITE(nn,'(I2)')  name_found
                ELSE IF ( name_found < 1000 ) THEN
                   WRITE(nn,'(I3)')  name_found
                ELSE
                   ! Make sur to increase nn size when adding more cases
                   CALL ipslerr_p(3, 'routing_diagnostic', &
                        'Non of the previous values can fit in the new char', &
                        'Add a new condition to deal with it', '')
                ENDIF
                sorted_names(ic) = TRIM(sorted_names(ic))//TRIM(nn)
                name_found = name_found - 1
             ENDIF
          ENDDO
       ENDIF
...
!! INPUT VARIABLES
    INTEGER(i_std), INTENT (in)                    :: nbpt                  !! Domain size  (unitless)
    REAL(r_std), DIMENSION(nbpt)                   :: contfrac
    INTEGER(i_std), DIMENSION(nbpt,NbNeighb), INTENT (in) :: neighbours            !!
