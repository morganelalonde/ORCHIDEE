...
!!
!! SVN		:
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE_2_2/ORCHIDEE/src_stomate/stomate_soilcarbon.f90 $ 
!! $Date: 2018-01-24 08:45:20 +0100 (Wed, 24 Jan 2018) $
!! $Revision: 4906 $
!! \n
!_ ================================================================================================================================
...
!_ ================================================================================================================================
  ...
  SUBROUTINE soilcarbon (npts, clay, &
       soilcarbon_input, control_temp, control_moist, veget_cov_max, &
...
!$OMP THREADPRIVATE(carbon_tau)
    ...
    INTEGER(i_std)                                        :: k,kk,m,j,ij      !! Indices (unitless)
      ...
      DO k = 1, ncarb ! Loop over carbon pools from which the flux comes
        ...
        ! Not crop
         IF ( natural(m) ) THEN
            ...
            decomp_rate_soilcarbon(:,m,k)=dt/carbon_tau(k) * &
                      control_moist(:,ibelow) * control_temp(:,ibelow)
         ! C3 crop
        ...
        IF ( k .EQ. iactive ) THEN
            ...
            decomp_rate_soilcarbon(:,m,k)=decomp_rate_soilcarbon(:,m,k)* ( un - flux_tot_coeff(3) * clay(:) ) 
        ENDIF
