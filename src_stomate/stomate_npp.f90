...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE_2_2/ORCHIDEE/src_stomate/stomate_npp.f90 $
!! $Date: 2019-08-05 11:11:40 +0200 (Mon, 05 Aug 2019) $
!! $Revision: 6151 $
!! \n
!_ ================================================================================================================================
...
!! 0 Variable and parameter declaration
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)           :: co2_to_bm        !! virtual gpp @tex $(gC.m^{-2})$ @endtex
    ...
    REAL(r_std), DIMENSION(npts,nparts)                       :: resp_growth_part !! Growth respiration of different plant parts
                                                                                  !! @tex $(gC.m^{-2}dt^{-1})$ @endtex		
    REAL(r_std), DIMENSION(npts,nparts)                       :: resp_growth_part_weighted !! Growth respiration of different plant
                                                                                           !! parts per total ground area weighted  
                                                                                           !! over PFT fractions 
                                                                                           !! @tex $(gC m^{-2}dt^{-1})$ @endtex
    REAL(r_std), DIMENSION(npts,nparts)                       :: resp_maint_part_weighted  !! Maintenance respiration of different 
                                                                                           !! plantparts per total ground area weighted  
                                                                                           !! over PFT fractions 
                                                                                           !! @tex $(gC m^{-2}dt^{-1})$ @endtex
...
!_ ================================================================================================================================
 ...
 !! 1. Initializations
    ...
    resp_growth(:,:) = zero
    npp(:,:) = zero
    resp_maint_part_weighted(:,:) = zero
    resp_growth_part_weighted(:,:) = zero
    ...
    DO j = 2,nvm	! Loop over # of PFTs
	  ...
	  ! then biomass associated with maintenance respiration is removed from allocatable biomass
       ...
       resp_growth_part(:,:) = frac_growthresp(j) * bm_alloc(:,j,:,icarbon) / dt
       bm_alloc(:,j,:,icarbon) = ( un - frac_growthresp(j) ) * bm_alloc(:,j,:,icarbon)
       ...
       DO k = 1, nparts
          ...
          resp_growth_part_weighted(:,k) = resp_growth_part_weighted(:,k) + resp_growth_part(:,k)*veget_cov_max(:,j)
          resp_maint_part_weighted(:,k) = resp_maint_part_weighted(:,k) + resp_maint_part(:,j,k)*veget_cov_max(:,j)
       ENDDO
 ...
 !! 5. Deal with negative biomasses
    ...
    !  biomass to a small value (min_stomate). For carbon budget to remain balanced, this creation of matter (carbon)
    !  is added to co2_to_bm. In previous versions, this carbon was taken from the autotrophic respiration. 
    DO k = 1, nparts	! Loop over # of plant parts
       ...
       DO j = 2,nvm	! Loop over # of PFTs
          ...
          WHERE ( biomass(:,j,k,icarbon) .LT. zero )
             ...
             co2_to_bm(:,j) = co2_to_bm(:,j) + bm_create(:,j) 
 ...
 !! 9. Write history files
    ...
    CALL xios_orchidee_send_field("resp_maint_part_weighted",resp_maint_part_weighted)
    CALL xios_orchidee_send_field("resp_growth_part_weighted",resp_growth_part_weighted)
