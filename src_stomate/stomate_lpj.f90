...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE_2_2/ORCHIDEE/src_stomate/stomate_lpj.f90 $
...
!!$    END DO ! Loop over # elements
  ...
  !! 17. Write history
    ...
    CALL xios_orchidee_send_field("NPP_STOMATE",npp_daily+co2_to_bm)
    CALL xios_orchidee_send_field("GPP",gpp_daily+co2_to_bm)
    ...
    CALL xios_orchidee_send_field("gpp_ipcc",SUM((gpp_daily+co2_to_bm)*veget_cov_max,dim=2)/1e3/one_day)
    ...
    CALL xios_orchidee_send_field("npp_ipcc",SUM((npp_daily+co2_to_bm)*veget_cov_max,dim=2)/1e3/one_day)

    vartmp(:)=zero
    DO j = 2, nvm
       IF ( .NOT. is_tree(j) .AND. natural(j) ) THEN
          vartmp(:) = vartmp(:) +( npp_daily(:,j)+co2_to_bm(:,j))*veget_cov_max(:,j)
       ENDIF
    ENDDO
    ...
    CALL xios_orchidee_send_field("nbp",(SUM((gpp_daily+co2_to_bm-(resp_maint+resp_growth+resp_hetero)-co2_fire) * &
    ...
    DO j=1,nvm
       IF (natural(j)) THEN
          ...
          npplut(:,id_psl) = npplut(:,id_psl) + &
               (npp_daily(:,j)+co2_to_bm(:,j))*veget_cov_max(:,j)/1e3/one_day
       ELSE
    ...
    CALL histwrite_p (hist_id_stomate, 'NPP', itime, &
         npp_daily+co2_to_bm, npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'GPP', itime, &
         gpp_daily+co2_to_bm, npts*nvm, horipft_index)
    ...
    IF ( hist_id_stomate_IPCC > 0 ) THEN
       ...
       CALL histwrite_p (hist_id_stomate_IPCC, "lai", itime, &
            vartmp, npts, hori_index)
       vartmp(:)=SUM((gpp_daily+co2_to_bm)*veget_cov_max,dim=2)/1e3/one_day
       ...
       CALL histwrite_p (hist_id_stomate_IPCC, "ra", itime, &
            vartmp, npts, hori_index)
       vartmp(:)=SUM((npp_daily+co2_to_bm)*veget_cov_max,dim=2)/1e3/one_day
