...
!                 haversine_diffangle : Computes the angle between two headings.
...
MODULE haversine
  ...
  PUBLIC :: haversine_reglatlontoploy, haversine_regxytoploy, haversine_singlepointploy, &
       ...
       &    haversine_laloarea, haversine_xyarea, haversine_diffangle, haversine_distance
...
!_ ==============================================================================================================================
  ...
  !!
  !!
  REAL(r_std) FUNCTION haversine_diffangle(x,y)
    !
    ! This function computes the smallest angle between 2 headings.
    ! A positive angle will be in mathematical positive direction (anti-clockwise)
    ! and a negative one is measured clockwise.
    !
    IMPLICIT NONE
    REAL(r_std), INTENT(IN) :: x, y
    !
    REAL(r_std) :: rx, ry
    !
    rx=haversine_dtor(x)
    ry=haversine_dtor(y)
    haversine_diffangle = haversine_rtod(atan2(sin(ry-rx), cos(ry-rx)))
    !
  END FUNCTION haversine_diffangle
  !!
  !!
  !!
